...
- 创建或修改任务后，先将原始评分标准编译为 Rubric Schema v2，再执行确定性校验和独立覆盖审计；只有审计通过后才能开始生成。
- 每道题独立生成参考答案。
- 答案生成阶段只加载已验证的结构化规则，不重复注入原始评分标准。
- 答案生成后自动按任务评分标准审核。
- 分数未达标时，只加载本轮缺失的评分项和修复建议继续重试，直到通过或达到重试上限。
- 支持查看当前生成题目、生成进度、耗时、得分、重试意见和最终状态。
- 支持导出生成结果。
- 支持通过 GitHub Actions 构建镜像并自动部署到服务器。

> [!NOTE]
> 评分标准编译和答案生成必须配置 `OPENAI_API_KEY`；只有答案审核在未配置模型密钥时提供确定性本地兜底。
...
1. 新建任务并填写评分标准。
2. 系统将原始评分标准编译为 Rubric Schema v2，执行确定性校验和独立覆盖审计。
3. 手动新增题目或上传 Word 文档。
4. 结构化评分规则审计通过后，人为点击开始任务。仍使用旧版评分规则的历史任务必须先重新分析评分标准，才能再次生成。
5. Worker 只使用已验证的结构化规则逐题生成参考答案，不再重复注入原始评分标准。
6. Worker 按已验证的结构化规则审核答案。
7. 未通过的答案只带着本轮缺失的评分项和结构化修复建议继续生成下一轮。
